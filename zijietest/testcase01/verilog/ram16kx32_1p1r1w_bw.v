...
//                                                                    
//   Memory Name:ram16kx32_1p1r1w_bw                                  
...
module ram16kx32_1p1r1w_bw ( Q, ADR, D, WEM, WE, ME, CLK, TEST1, RME, RM);
...
assign  ADR_buf[12] =  ADR[12];
assign  ADR_buf[13] =  ADR[13];
wire [31:0] D_buf;
assign  D_buf[0] =  D[0];
assign  D_buf[1] =  D[1];
assign  D_buf[2] =  D[2];
...
`endif
wire ADR_valid;
//assign ADR_valid =  (ADR_buf >= 14'b00000000000000 && ADR_buf <= 14'b11111111111111 )?1'b1:1'b0;
...
// Timing Checks
  ...
  $setuphold (posedge CLK  &&& ENADR, posedge ADR[12], Tac, Tcax, notif_adr );
  $setuphold (posedge CLK  &&& ENADR, negedge ADR[12], Tac, Tcax, notif_adr );
  $setuphold (posedge CLK  &&& ENADR, posedge ADR[13], Tac, Tcax, notif_adr );
  $setuphold (posedge CLK  &&& ENADR, negedge ADR[13], Tac, Tcax, notif_adr );
...
`ifdef VIRAGE_FAST_VERILOG
module generic_behav_ram16kx32_1p1r1w_bw (  Q, ADR, D, WEM, WE, ME, CLK, TEST1, RME, RM );
`else
module generic_behav_ram16kx32_1p1r1w_bw (  Q, ADR, D, WEM, WE, ME, CLK, TEST1, RME, RM,notif_adr, notif_d, notif_wem, notif_we, notif_me, notif_clk, notif_test1, notif_rme, notif_rm );
`endif // endif of VIRAGE_FAST_VERILOG

parameter PreloadFilename = "init.file";
parameter MES_ALL = "ON";
parameter words = 16384, bits = 32, addrbits = 14, O_delay=0.0, timingmode = 1, wembits=32, rmbits=4;
...
reg[ 31:0] mem_core_array_read [0:16383];
